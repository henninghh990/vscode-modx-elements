...
async function ensureStorageFile(context) {
...
async function getSitesFile(context) {
...
async function updateSite(context, oldBaseUrl, newData) {
  const sites = await loadSites(context);
...
async function removeSite(context, baseUrl) {
  const sites = await loadSites(context);
...
async function storeToken(context, tokenKey, token) {
  await context.secrets.store(tokenKey, token);
}
async function getToken(context, tokenKey) {
...
var ModxNode = class extends vscode2.TreeItem {
...
var SiteNode = class extends ModxNode {
  constructor(siteCfg, context) {
    super(siteCfg.name, vscode2.TreeItemCollapsibleState.Collapsed);
    this.siteCfg = siteCfg;
    this.context = context;
    this.site = new ModxSite(siteCfg, context);
    this.tooltip = siteCfg.baseUrl;
    this.contextValue = "modxSite";
    this.iconPath = new vscode2.ThemeIcon("globe");
  }
  getCategories() {
...
var CategoryNode = class extends ModxNode {
  constructor(parent, type) {
    super(displayName(type).name, vscode2.TreeItemCollapsibleState.Collapsed);
    this.parent = parent;
    this.type = type;
    this.site = parent.site;
    const stats = displayName(type);
    this.contextValue = "modxCategory";
    this.iconPath = new vscode2.ThemeIcon(stats.icon);
  ...
  async fetchElements() {
    try {
      ...
      vscode2.window.showErrorMessage(`Failed to fetch ${this.type} from ${this.site?.name}: ${e}`);
      return [];
    }
  }
  async getElements(filterQuery) {
  ...
  filter(list, query) {
    ...
    const items = filtered.map((el) => {
      ...
      const lbl = start >= 0 ? { label: labelText, highlights: [[start, start + query.length]] } : { label: labelText };
      el.name = lbl.label;
      return el;
    });
    return items;
...
var ElementNode = class extends ModxNode {
  constructor(parent, element) {
    super(element.name, vscode2.TreeItemCollapsibleState.None);
    this.parent = parent;
    this.element = element;
    this.site = parent.site;
    this.contextValue = "modxElement";
    this.tooltip = `${element.name} (${parent.type})`;
    const ext = parent.type === "modSnippet" || parent.type === "modPlugin" ? ".php" : ".html";
    this.resourceUri = vscode2.Uri.from({
      scheme: "modx",
...
function displayName(modxType) {
  switch (modxType) {
    case "modSnippet":
      return { name: "Snippets", icon: "code" };
    case "modChunk":
      return { name: "Chunks", icon: "extensions" };
    case "modTemplate":
      return { name: "Templates", icon: "split-vertical" };
    case "modPlugin":
      return { name: "Plugins", icon: "gear" };
    default:
      return { name: modxType, icon: "question" };
  }
}
...
var ModxFs = class {
  ...
  _em = new vscode3.EventEmitter();
  client = null;
  sitename = "";
  type = "";
  id = null;
  name = "";
  onDidChangeFile = this._em.event;
  async init(uri) {
    this.parse(uri);
  ...
  parse(uri) {
    ...
    if (!sitename || !type || !idStr || !rest) {
      throw vscode3.FileSystemError.FileNotFound(uri);
    }
  ...
  async writeFile(uri, content, options) {
    ...
    this._em.fire([{ type: vscode3.FileChangeType.Changed, uri }]);
  }
...
// src/siteTreeProvider.ts
var vscode4 = __toESM(require("vscode"));
var SiteTreeProvider = class {
  constructor(context) {
    this.context = context;
  }
  _em = new vscode4.EventEmitter();
  onDidChangeTreeData = this._em.event;
  ...
  async getChildren(element) {
    if (!element) {
      ...
      if (!sites || sites.length === 0) {
        vscode4.commands.executeCommand("setContext", "vscode-modx-elements.noSitesConfigured", true);
        return [];
      }
      vscode4.commands.executeCommand("setContext", "vscode-modx-elements.noSitesConfigured", false);
...
// src/webviews/addSiteWebview.ts
var vscode5 = __toESM(require("vscode"));
async function openAddOrEditSiteWebview(context, provider, existingSite) {
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    try {
      if (msg.type === "ping") {
        ...
        const url2 = normalizeApiUrl(baseUrl, apiUrl);
        const headers = token ? { Authorization: `Bearer ${token}` } : void 0;
        console.log(url2);
        try {
          const res = await axios_default.get(url2, { headers, timeout: 2e3 });
          console.log(res);
          if (!res) {
          ...
          vscode5.window.showErrorMessage(`Failed to ping: ${e}`);
      ...
      if (msg.type === "submit") {
        ...
        if (existingSite) {
          ...
          vscode5.window.showInformationMessage(`Updated site: ${site.name}`);
        } else {
          await addSite(context, site);
          vscode5.window.showInformationMessage(`Added site: ${site.name}`);
        }
      ...
      if (msg.type === "delete") {
        ...
        vscode5.window.showInformationMessage(`Deleted`);
        provider.refresh();
        panel.dispose();
      }
      if (msg.type === "cancel") {
...
function getHtml(webview, nonce, site) {
  ...
  <div class="wrap">
    ...
    <form id="f">
      ...
      <div class="bs-row">
            ...
            </div>` : `<input type="hidden" id="delete" />`}
...
function normalizeApiUrl(base, api) {
  if (/^https?:\/\//i.test(api)) {
    return api;
  }
  return `${trimRightSlash(base)}/${normalizeApiPath(api)}ping`;
}
function sanitizeForm(p) {
  const name = String(p.name ?? "").trim();
  const baseUrl = String(p.baseUrl ?? "").trim();
  const apiUrl = String(p.apiUrl ?? "").trim();
  const token = String(p.token ?? "").trim();
  const elements = Array.isArray(p.elements) && p.elements.length ? p.elements.map((c) => String(c)) : ["modSnippet", "modChunk", "modTemplate", "modPlugin"];
...
async function activate(context) {
  ...
  const fs2 = new ModxFs(context);
  context.subscriptions.push(
    vscode6.workspace.registerFileSystemProvider("modx", fs2, { isCaseSensitive: true })
  );
  const treeView = vscode6.window.createTreeView("modx-elements-tree", {
    treeDataProvider: provider
  });
  context.subscriptions.push(treeView);
  const refreshCommand = vscode6.commands.registerCommand("vscode-modx-elements.refreshElements", () => {
    provider.refresh();
  });
  context.subscriptions.push(refreshCommand);
  const newElementCommand = vscode6.commands.registerCommand("vscode-modx-elements.newElement", async (node) => {
    console.log(node);
  ...
  const searchCommand = vscode6.commands.registerCommand("vscode-modx-elements.searchElements", async () => {
    const value = await vscode6.window.showInputBox({
    ...
    vscode6.commands.executeCommand("setContext", "vscode-modx-elements.hasActiveFilter", !!value?.trim());
  });
  context.subscriptions.push(searchCommand);
  const clearSearchCommand = vscode6.commands.registerCommand("vscode-modx-elements.clearSearch", () => {
    provider.setFilter("");
    vscode6.commands.executeCommand("setContext", "vscode-modx-elements.hasActiveFilter", false);
  });
  context.subscriptions.push(clearSearchCommand);
  const addSiteCommand = vscode6.commands.registerCommand(
    "vscode-modx-elements.addModxSite",
    async () => {
      await openAddOrEditSiteWebview(context, provider);
    }
  );
  context.subscriptions.push(addSiteCommand);
  const editSiteCommand = vscode6.commands.registerCommand("vscode-modx-elements.editModxSite", async (node) => {
    if (!node || !node.siteCfg) {
      vscode6.window.showWarningMessage("No MODX site selected.");
      return;
    }
    await openAddOrEditSiteWebview(context, provider, node.siteCfg);
  });
  context.subscriptions.push(editSiteCommand);
  const deleteSiteCommand = vscode6.commands.registerCommand("vscode-modx-elements.deleteModxSite", async (node) => {
